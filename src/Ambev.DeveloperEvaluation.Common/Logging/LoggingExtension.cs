...
public static class LoggingExtension
{
    ...
    /// Never exclude errors, warnings, or fatal logs - only filter Information level logs for health checks.
    /// </summary>
    static readonly Func<LogEvent, bool> _filterPredicate = exclusionPredicate =>
    {
        // Never filter errors, warnings, or fatal logs
        if (exclusionPredicate.Level >= LogEventLevel.Warning) return true;

        // Only filter Information level logs for health checks with status 200
    ...
    public static WebApplicationBuilder AddDefaultLogging(this WebApplicationBuilder builder)
    {
        ...
        builder.Host.UseSerilog((hostingContext, loggerConfiguration) =>
        {
            loggerConfiguration
                ...
                .MinimumLevel.Is(LogEventLevel.Debug) // Set minimum level to Debug to capture all errors
